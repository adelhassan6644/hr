...
class AuthProvider extends ChangeNotifier with BaseViewModel {
 ...
 logIn( ) async {
  ...
  _isLoading = true;
  notifyListeners();
  Either<ServerFailure, Response> response= await authRepo.logIn(email: _emailTEC.text.trim(), password: _passwordTEC.text.trim());
  response.fold((fail)  {
 ...
 getOTP() async {
   try{
     ...
     _isLoading = true;
     notifyListeners();
     Either<ServerFailure, Response> response= await authRepo.getVerificationCode(email: _emailTEC.text.trim(),);
     response.fold((fail)  {
       CustomSnackBar.showSnackBar(
           notification: AppNotification(
               message: fail.error,
 ...
 sendOTP() async {
   try{
     ...
     _isLoading = true;
     notifyListeners();
     Either<ServerFailure, Response> response= await authRepo.sendVerificationCode(code: _codeTEC.text.trim(),);
     response.fold((fail)  {
       CustomSnackBar.showSnackBar(
           notification: AppNotification(
               message: fail.error,
 ...
 reSendOTP() async {
   await authRepo.getVerificationCode(email: _emailTEC.text.trim(),);
 }
 ...
 resetPassword() async {
    try{
      ...
      _isLoading = true;
      notifyListeners();
      Either<ServerFailure, Response> response= await authRepo.resetPassword(newPassword: _newPasswordTEC.text.trim(),);
      response.fold((fail)  {
        CustomSnackBar.showSnackBar(
            notification: AppNotification(
                message: fail.error,
  ...
  updatePassword() async {
    try{
      ...
      _isLoading = true;
      notifyListeners();
      Either<ServerFailure, Response> response= await authRepo.updatePassword(email: _emailTEC.text.trim(),
